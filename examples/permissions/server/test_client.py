...
p = optparse.OptionParser(
    ...
    "are supported: %s" % ', '.join([a.__name__ for a in actions]))
...
p.add_option(
    '-b', '--blob', dest='blob', help="a blob string (used by update_blob)")
p.add_option(
    '-d',
    '--description',
    default='',
...
p.add_option(
    ...
    default=False,
    dest='verbose',
    help="display the progress of the RPC")
...
proxy = xmlrpc.client.ServerProxy(
    uri='http://%s:%d' % (opts.addr, opts.port), verbose=opts.verbose)

try:
    result = getattr(proxy, action.__name__)(*action_args)
