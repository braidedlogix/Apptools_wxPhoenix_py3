...
class BindingsColumn(ObjectColumn):
    ...
    def get_editor(self, object):
        return EnumEditor(values=[
            dnic.choice_value
            for dnic in object.data_name.items[self.index]
...
std_columns = [
    ResolvedColumn(
        ...
        cell_color=0xFF8080), OptionalColumn(
            name='optional',
            label='*',
            editable=False,
            width=20,
            horizontal_alignment='center'), ObjectColumn(
                name='description', editor=TextEditor(), width=0.47)
]
...
class TemplateDataNames(HasPrivateTraits):
    ...
    def _get_table_columns(self):
        ...
        if n == 1:
            return std_columns + [
                BindingsColumn(
                    name='value0', label='Name', width=0.43)
            ]
        width = 0.43 / n
        return (std_columns + [
            BindingsColumn(
...
def _get_virtual_data(self, name):
    return self.data_name.items[self.trait(name)
                                .index].data_name_item_choice.choice_value
