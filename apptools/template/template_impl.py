...
class Template(HasPrivateTraits):
    ...
    def names_from_template(self):
        ...
        # objects:
        for value in list(self.get(template='instance').values()):
        ...
        for value in list(self.get(template='datasource').values()):
            if isinstance(value, list):
                for v in value:
    ...
    def template_from_object(self):
        ...
        for trait in list(self.traits(template=is_none).values()):
        ...
        for name, value in list(contents.items()):
            if isinstance(value, list):
